...
func (kp *KafkaProducer) SendwithEbaseRequestID(msg_str string, EbaseRequestID string) (partition int32, offset int64, err error) {
	...
	kafka_msg := &sarama.ProducerMessage{
		...
		Headers: []sarama.RecordHeader{
			{
				Key:   []byte("EbaseRequestID"),
				Value: []byte(EbaseRequestID),
			},
		},
	}
	partition, offset, err = (*kp.Sp).SendMessage(kafka_msg)
